...
<nav aria-label="Page navigation example ">
    <ul class="pagination d-flex justify-content-center mt-4">
        <li class="page-item  <?= $pageNum == 1 ? 'disabled' : '' ?>"><a class="page-link" href="<?= $link?>1">Trang đầu</a></li>
        <?php
        $start = 1;
        if ($pageNum >= 6) {
            $start = $pageNum - 4;
        ...
        for ($start; $start <= $end; $start++) { ?>
            <li class="page-item <?= $pageNum == $start ? 'disabled' : '' ?>"><a class="page-link " href="<?= ROOT_URL . 'admin/san-pham?page=' . $start ?>"><?= $start ?></a></li>
        <?php }
        ?>
        <li class="page-item <?= $pageNum == $end ? 'disabled' : '' ?>"><a class="page-link" href="<?= ROOT_URL . 'admin/san-pham?page=' . $tongSoTrang ?>">Trang cuối</a></li>
    </ul>
</nav>
